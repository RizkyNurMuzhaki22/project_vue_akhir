<template>
  <div class="flex flex-col items-center border-b  bg-blue-500 py-4 sm:flex-row sm:px-10 lg:px-20 xl:px-32">
      <a href="#" class="text-2xl font-bold text-gray-800 text-white" >Order Summary</a>
        ...
        <div class="relative">
          <ul class="relative flex w-full items-center justify-between space-x-2 sm:space-x-4">
            <li class="flex items-center space-x-3 text-left sm:space-x-4">
              ...
              <span class="font-semibold text-gray-500 text-white">Shop</span>
            </li>
           ...
           <li class="flex items-center space-x-3 text-left sm:space-x-4">
              ...
              <span class="font-semibold text-gray-900 text-white">Shipping</span>
            </li>
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
            </svg>
             <li class="flex items-center space-x-3 text-left sm:space-x-4">
              ...
              <span class="font-semibold text-gray-900 text-white">Done</span>
            </li>
          </ul>
        </div>
      </div>
      ...
      <!-- component -->
<div class="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto bg-indigo-500">
  <!--- more free and premium Tailwind CSS components at https://tailwinduikit.com/ --->

  <div class="flex justify-start item-start space-y-2 flex-col">
    <h1 class="text-3xl dark:text-white lg:text-4xl font-semibold leading-7 lg:leading-9 text-gray-800 text-white">Kode Pesanan : {{ getOrder.code }}</h1>
    <p class="text-base dark:text-gray-300 font-medium leading-6 text-gray-600"> Terimakasih atas kepercayaan Tn./Ny. {{ getOrder.user.name }}</p>
  </div>
  <div class="mt-10 flex flex-col xl:flex-row jusitfy-center items-stretch w-full xl:space-x-8 space-y-4 md:space-y-6 xl:space-y-0">
    <div class="flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8">
      <div class="flex flex-col justify-start items-start dark:bg-gray-800 bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full">
        <p class="text-lg md:text-xl dark:text-white font-semibold leading-6 xl:leading-5 text-gray-800">Informasi Pesanan :</p>
        <div class="mt-4 md:mt-6 flex flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full" v-for="cart in getCart" :key="cart.cart_id">
          ...
          <div class="flex justify-between items-start w-full">
            <div class="flex justify-center items-center space-x-4">
              ...
              <div class="flex flex-col justify-start items-center">
                <p class="text-lg leading-6 dark:text-white font-semibold text-gray-800">Metode Pengiriman :<br /><span class="font-normal">{{ getOrder.orders[0].payment_type }}</span></p>
              </div>
            </div>
            <p class="text-lg font-semibold leading-6 dark:text-white text-gray-800">{{ getOrder.orders[0].delivery_type }}</p>
          </div>
        ...
        <div class="flex flex-col justify-start items-start flex-shrink-0">
          <div class="flex justify-center w-full md:justify-start items-center space-x-4 py-8 border-b border-gray-200">
            ...
            <div class="flex justify-start items-start flex-col space-y-2">
              ...
              <p class="text-sm dark:text-gray-300 leading-5 text-gray-600">No Telp : {{  getOrder.shipping_address.phone }} </p>
            </div>
          </div>
        ...
        <div class="flex justify-between xl:h-full items-stretch w-full flex-col mt-6 md:mt-0">
          <div class="flex justify-center md:justify-start xl:flex-col flex-col md:space-x-6 lg:space-x-8 xl:space-x-0 space-y-4 xl:space-y-12 md:space-y-0 md:flex-row items-center md:items-start">
            <div class="flex justify-center md:justify-start items-center md:items-start flex-col space-y-4 xl:mt-8">
              <p class="text-base dark:text-white font-semibold leading-4 text-center md:text-left text-gray-800">Alamat Pengirim :</p>
              <p class="w-48 lg:w-full dark:text-gray-300 xl:w-48 text-center md:text-left text-sm leading-5 text-gray-600">{{ getOrder.shipping_address.address }}, {{  getOrder.shipping_address.city }}, {{  getOrder.shipping_address.state }}, {{  getOrder.shipping_address.country }}, {{  getOrder.shipping_address.postal_code }}</p>
            </div>
            <div class="flex justify-center md:justify-start items-center md:items-start flex-col space-y-4">
              <p class="text-base dark:text-white font-semibold leading-4 text-center md:text-left text-gray-800">Alamat Penerima :</p>
              <p class="w-48 lg:w-full dark:text-gray-300 xl:w-48 text-center md:text-left text-sm leading-5 text-gray-600">{{ getOrder.shipping_address.address }}, {{  getOrder.shipping_address.city }}, {{  getOrder.shipping_address.state }}, {{  getOrder.shipping_address.country }}, {{  getOrder.shipping_address.postal_code }}</p>
            </div>
          </div>
 ...
 import { mapGetters, mapState } from 'vuex';
    export default {
      ...
      computed: {
        ...
        ...mapGetters('cart', ['getCart', 'fetchCart'])
      },
      created() {
